...
import com.readboy.Q.HappyDoodle.util.ImageWorker.ImageWorkerAdapter;
 ...
 */
public class Images {
	private static final String TAG = "lqn-Images";
    /**
    ...
    /**
     * Simple static adapter to use for resource image thumbnails. add by css
     */
    public final static ImageWorkerAdapter imageThumbWorkerResAdapter = new ImageWorkerAdapter() {
        @Override
        public Object getItem(int num) {
        	Log.e(TAG, "num="+num);
        	return OpusSetActivity.getOpusFilesPath().get(num);
            //return Images.imageThumbRes[num];
        }

        @Override
        public int getSize() {
            //return Images.imageThumbRes.length;
        	return OpusSetActivity.getOpusFilesPath().size();
        }
    };
}
